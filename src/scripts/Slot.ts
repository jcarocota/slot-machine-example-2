...
const calculateTagSlot: () => string = () => {
    const numFruit:number = Math.ceil(Math.random()*globals.numberOfSlotsInTextureSheet);
...
export const createSlot : () => Slot | undefined = () : Slot | undefined => {
    ...
    const sprite: PIXI.Sprite = new PIXI.Sprite(globals.slotTextureSheet.textures[tagSlot]);
